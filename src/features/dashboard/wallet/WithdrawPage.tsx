...
export function WithdrawPage() {
  ...
  const bank = selectedBank ?? banks[0] ?? null;
    ...
    return (
      <WalletShell title="Withdraw" eyebrow="To bank">
        <WalletCard className="text-center">
          ...
          <h2 className="mt-4 font-display text-lg font-semibold">No bank account yet</h2>
          <p className="mt-1 text-sm text-muted-foreground">
            Add a bank account to start withdrawing your funds.
          </p>
          <Button
            onClick={() => navigate({ to: "/dashboard/wallet/withdraw/add-bank" })}
            size="lg"
          ...
          <div className="flex items-center justify-between rounded-2xl border border-border bg-background p-4">
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="truncate text-xs text-muted-foreground">{bank.bank_name}</p>
              </div>
            </div>
            <span className="font-mono text-sm tabular-nums">{bank.account_number}</span>
          </div>
        )}
