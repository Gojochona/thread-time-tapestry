...
const transactions = [
  { id: "tx_01", type: "credit", title: "Wallet funded", meta: "Wema Bank · ****5678", amount: 50_000, date: "Today, 09:42", icon: Banknote },
  { id: "tx_02", type: "escrow", title: "Escrow · Aso Ebi order", meta: "Freddy Han · #A2841", amount: -25_000, date: "Yesterday, 18:10", icon: Lock },
  { id: "tx_03", type: "debit", title: "Fabric purchase", meta: "Royal Aso Oke", amount: -12_500, date: "Apr 14, 14:22", icon: Shirt },
  { id: "tx_04", type: "credit", title: "Refund · cancelled order", meta: "Marco Bianchi", amount: 18_000, date: "Apr 12, 11:05", icon: RefreshCcw },
  { id: "tx_05", type: "debit", title: "Tailor payout released", meta: "Amaka Okafor", amount: -32_000, date: "Apr 10, 09:30", icon: Scissors },
];
...
export function WalletPage() {
  ...
  const [filter, setFilter] = useState<"all" | "credit" | "debit" | "escrow">("all");
    ...
    <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="relative grid gap-8 lg:grid-cols-[1.5fr_1fr] lg:items-end">
          <div>
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs uppercase tracking-wider text-primary-foreground/70">Wallet</p>
                <h1 className="font-display text-2xl font-bold sm:text-3xl">Manage your funds & escrow</h1>
              </div>
            </div>
            ...
            <div className="mt-6">
              ...
              <div className="mt-1 flex items-center gap-3">
                <div className="font-display text-4xl font-bold sm:text-5xl">{mask(fmt(available))}</div>
                <button
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="rounded-2xl bg-white/10 p-4 backdrop-blur border border-white/15">
              ...
              <div className="mt-2 font-display text-2xl font-bold">{mask(fmt(total))}</div>
              <p className="mt-1 text-[11px] text-primary-foreground/60">Available + locked</p>
            </div>
          </div>
        </div>
      </motion.div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card delay={0.1} className="lg:col-span-2">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h2 className="font-display text-xl font-semibold">Balance history</h2>
              ...
              {["7D", "30D", "90D"].map((r, i) => (
                <button
                  ...
                  className={`rounded-full px-3 py-1 font-medium transition-colors ${
                    i === 1 ? "bg-card text-foreground shadow-soft" : "text-muted-foreground hover:text-foreground"
                  }`}
                >
                  {r}
                </button>
          ...
          <div className="mt-5 h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={balanceHistory} margin={{ left: 0, right: 8, top: 8, bottom: 0 }}>
                <defs>
                ...
                />
                <Area type="monotone" dataKey="balance" stroke="var(--color-primary)" strokeWidth={2.5} fill="url(#balanceFill)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </Card>
        ...
        <Card delay={0.15}>
          <div className="flex items-center justify-between">
            <h2 className="font-display text-xl font-semibold">Virtual account</h2>
            <span className="rounded-full bg-success/15 px-2 py-0.5 text-[11px] font-semibold text-success">Active</span>
          </div>
          <p className="mt-1 text-xs text-muted-foreground">Send funds here to top up instantly.</p>
            ...
            <div className="rounded-2xl border border-border bg-background p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-1 font-display text-xl font-bold tracking-wider">8012345678</p>
                </div>
                <button
          ...
          <p className="mt-4 text-[11px] text-muted-foreground">
            Funds sent to this account are credited automatically within minutes.
          </p>
        </Card>
      </div>
      ...
      <Card delay={0.2}>
        ...
        <ul className="mt-4 divide-y divide-border">
          {filtered.length === 0 && (
            <li className="py-10 text-center text-sm text-muted-foreground">No transactions match your filter.</li>
          )}
          {filtered.map((t) => {
            const Icon = t.icon;
