...
import { CreditCard, Building2, Wallet as WalletIcon, Plus } from "lucide-react";
import {
  getSavedCards,
  topUpWithCard,
...
import { WalletShell, WalletCard, FieldLabel, PaystackBadge } from "./WalletShell";
...
function CardTopUp() {
  ...
  const { data: cards = [] } = useQuery({ queryKey: ["wallet-cards"], queryFn: getSavedCards });

  const mutation = useMutation({
    mutationFn: topUpWithCard,
    onSuccess: () =>
      navigate({
        to: "/dashboard/wallet/success",
    ...
    <div className="space-y-5">
        ...
        <div className="space-y-2">
          ...
          {cards.map((c) => (
            ...
            >
              ...
              <span className="text-xs uppercase text-muted-foreground">{c.brand}</span>
            </div>
          ))}
        </div>
      )}
      ...
      <div>
        ...
        <div className="flex items-center gap-2 rounded-2xl border border-border bg-background px-4 py-3 focus-within:border-primary">
          <span className="text-sm font-semibold text-muted-foreground">NGN</span>
          <input
      ...
      <label className="flex items-center gap-2 text-sm">
        <input
          ...
          onChange={(e) => setForm((f) => ({ ...f, save_card: e.target.checked }))}
      ...
      <Button
        onClick={() => mutation.mutate({ ...form, amount: Number(amountStr.replace(/,/g, "")) })}
        disabled={!isReady || mutation.isPending}
        size="lg"
        className="w-full"
      >
        <Plus size={16} />
        {mutation.isPending ? "Processing…" : `Top up ${amountStr ? `₦${amountStr}` : ""}`}
      </Button>
    ...
    <div className="space-y-5">
      <div className="rounded-2xl border border-dashed border-border bg-background p-5 text-center">
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          Use your Wema Bank virtual account on the wallet page to credit funds instantly.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        ...
        <div className="rounded-2xl border border-border bg-background p-4">
          <p className="text-[11px] uppercase text-muted-foreground">Account no.</p>
          <p className="mt-1 font-display text-lg font-bold tracking-wider">8012345678</p>
        </div>
        <div className="rounded-2xl border border-border bg-background p-4">
