...
export function AddBankPage() {
  ...
  const { data: bankList = [] } = useQuery({ queryKey: ["bank-list"], queryFn: getBankList });
  ...
  function handleAccountNumber(val: string) {
    ...
    setResolvedName(clean.length === 10 && form.bank_code ? "Ismaila Sarr" : null);
  }

  const isReady =
    form.bank_code !== "" && form.account_number.length === 10 && form.password.trim() !== "";
    ...
    >
      <WalletCard delay={0.05} className="space-y-5">
              ...
              {bankList.map((b) => (
                <button
                  ...
                  onClick={() => {
                    ...
                    setBankOpen(false);
                    if (form.account_number.length === 10) setResolvedName("Ismaila Sarr");
                  }}
                  className="flex w-full items-center justify-between px-4 py-3 text-left text-sm hover:bg-muted"
                >
                  <span>{b.name}</span>
                  {form.bank_code === b.code && <Check size={14} className="text-primary" />}
                </button>
              ))}
            </div>
          )}
        ...
        <div>
          ...
          />
          {resolvedName && (
            <p className="mt-1.5 text-right text-xs font-medium text-success">{resolvedName}</p>
          )}
        </div>
