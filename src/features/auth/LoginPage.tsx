    ...
    <div className="grid min-h-screen lg:grid-cols-2">
      ...
      <div className="relative flex items-center justify-center px-6 py-10">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
          ...
          <p className="mt-2 text-muted-foreground">Sign in to continue your tailoring journey.</p>

          <button type="button" className="mt-8 flex h-12 w-full items-center justify-center gap-3 rounded-full border border-border bg-card text-sm font-medium hover:bg-muted transition-all">
          ...
          <form className="space-y-4" onSubmit={onSubmit}>
            ...
            <div className="flex items-center justify-between text-sm">
              <label className="inline-flex items-center gap-2 text-muted-foreground">
                <input type="checkbox" className="h-4 w-4 rounded border-border accent-[oklch(0.34_0.15_264)]" /> Remember me
              </label>
          ...
          <p className="mt-6 text-center text-sm text-muted-foreground">
            Don't have an account? <Link to="/signup" className="font-semibold text-primary hover:underline">Create one</Link>
          </p>
        </motion.div>
      </div>
    </div>
...
export function AuthSide() {
  return (
    <div className="relative hidden overflow-hidden lg:block">
      <img src={showcase} alt="" className="absolute inset-0 h-full w-full object-cover" />
      ...
      <div className="relative flex h-full flex-col justify-between p-12 text-primary-foreground">
        ...
        <div>
          <h2 className="font-display text-5xl font-bold leading-tight">
            Custom tailoring,<br /><span className="text-accent italic">made personal.</span>
          </h2>
          <p className="mt-6 max-w-md text-primary-foreground/80">Join thousands of customers ordering bespoke outfits from verified master tailors.</p>
          <ul className="mt-8 space-y-3">
            {["Verified master tailors", "Secure escrow payments", "Real-time order tracking", "Free re-tailoring guarantee"].map((f) => (
              <li key={f} className="flex items-center gap-3 text-sm">
                <span className="flex h-6 w-6 items-center justify-center rounded-full gradient-accent text-primary"><Check size={14} /></span>
                {f}
              </li>
            ))}
          </ul>
        </div>
        <div className="text-xs text-primary-foreground/60">© 2026 i-sew. All rights reserved.</div>
      </div>
    </div>
  );
}
...
function GoogleIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 48 48" aria-hidden="true">
      ...
      <path fill="#4CAF50" d="M24 43.5c5 0 9.5-1.7 13-4.7l-6-5.1c-1.9 1.4-4.3 2.3-7 2.3-5.3 0-9.7-3.4-11.3-8l-6.5 5C9.5 39.1 16.2 43.5 24 43.5z"/>
